...
<div class="formModif">
	...
	<form action="modifEngin.php?id_Engins=<?= $_GET['id_Engins'] ?>" method="POST" enctype="multipart/form-data">
      ...
      foreach($typesListe as $types){ ?>
				<option value="<?= $types->id_types?> " <?= ($types->id_types == $selected)? 'selected': '' ?>>
					<?= $types->id_types . ' . ' . $types->nomTypes?></option><?php
      } ?>
			</select>
			<!--message d'erreur-->
		...
		<div class="form-group">
			...
			<input oninput="this.value = this.value.toUpperCase()" class="form-control" id="numeroEngin"
				<?= count($formErrors) > 0 ? (isset($formErrors['numeroEngin']) ? 'is-invalid' : 'is-valid') : '' ?>value="<?= !empty($_POST['numeroEngin']) ? $_POST['numeroEngin'] : $enginsInfo->numeroEngin ?>"
				type="text" name="numeroEngin" />
			<!--message d'erreur-->
		...
		<div class="form-group">
			...
			<input oninput="this.value = this.value.toUpperCase()" class="form-control" id="nomEquipements"
				<?= count($formErrors) > 0 ? (isset($formErrors['nomEquipements']) ? 'is-invalid' : 'is-valid') : '' ?>value="<?= isset($_POST['nomEquipements']) ? $_POST['nomEquipements'] : $enginsInfo->nomEquipements ?>"
				type="text" name="nomEquipements" />
			<!--message d'erreur-->
      ...
      foreach($statutListe as $statut){ ?>
				<option value="<?= $statut->id_statut ?>"
					<?= ($statut->id_statut == $statutSelected)? "selected": "" ?>>
					<?= $statut->id_statut . ' . ' . $statut->nomStatut ?></option><?php
      } ?>
			</select>
		...
		<div class="form-group">
			...
			<input class="form-control" id="dernierRevision"
				<?= count($formErrors) > 0 ? (isset($formErrors['dernierRevision']) ? 'is-invalid' : 'is-valid') : '' ?>value="<?= isset($_POST['dernierRevision']) ? $_POST['dernierRevision'] : $enginsInfo->dernierRevision ?>"
				type="date" name="dernierRevision" />
			<!--message d'erreur-->
		...
		<div class="form-group">
			...
			<input class="form-control" id="prochainRevision"
				<?= count($formErrors) > 0 ? (isset($formErrors['prochainRevision']) ? 'is-invalid' : 'is-valid') : '' ?>value="<?= isset($_POST['prochainRevision']) ? $_POST['prochainRevision'] : $enginsInfo->prochainRevision ?>"
				type="date" name="prochainRevision" />
			<!--message d'erreur-->
			<p class="errorForm"><?= isset($formErrors['prochainRevision']) ? $formErrors['prochainRevision'] : '' ?>
			</p>
		</div>
		...
		<div class="form-group">
			...
			<input class="form-control" id="km_reel"
				<?= count($formErrors) > 0 ? (isset($formErrors['km_reel']) ? 'is-invalid' : 'is-valid') : '' ?>value="<?= isset($_POST['km_reel']) ? $_POST['km_reel'] : $enginsInfo->km_reel ?>"
				type="number" name="km_reel" />
			<!--message d'erreur-->
		...
		<div class="form-group">
			...
			<input class="form-control" id="horametre"
				<?= count($formErrors) > 0 ? (isset($formErrors['horametre']) ? 'is-invalid' : 'is-valid') : ''?>value="<?= isset($_POST['horametre']) ? $_POST['horametre'] : $enginsInfo->horametre?>"
				type="time" name="horametre" />
      ...
      foreach($clientsListe as $clients){ ?>
				<option value="<?= $clients->id_Clients ?>"
					<?= ($clients->id_Clients == $selectedClient)? "selected": "" ?>>
					<?= $clients->id_Clients . ' . ' . $clients->nomClients ?></option><?php
      } ?>
			</select>
			<!--message d'erreur-->
		...
		<div class="form-group">
			...
			<input id="image"
				class="form-control <?= count($formErrors) > 0 ? (isset($formErrors['image']) ? 'is-invalid' : 'is-valid') : '' ?>"
				value="<?= isset($_POST['image']) ? $_POST['image'] :'' ?>" type="file" name="image" />
			<!--message d'erreur-->
