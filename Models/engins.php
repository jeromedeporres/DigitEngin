...
//$this-> : permet d'acceder aux attributs de l'instance qui est en cours
			'INSERT INTO `engins` ( `numeroEngin`,`id_equipements`, `id_statut`,`dernierRevision`, `km_reel` , `heure_jour`, `horametre`, `prochainRevision`,`id_Clients`,`image`,`id_types`)
        VALUES( :numeroEngin, :id_equipements, :id_statut,:dernierRevision, :km_reel, :heure_jour, :horametre, :prochainRevision, :id_Clients, :image,:id_types)'
        );
        $addEnginsQuery->bindvalue(':numeroEngin', $this->numeroEngin, PDO::PARAM_STR);
        $addEnginsQuery->bindvalue(':id_equipements', $this->id_equipements, PDO::PARAM_INT);
...
public function getEnginsList($orderBy) {
    ...
    /*     'SELECT `engins`.`id_Engins`, `nomTypes`, `nomClients` , `numeroEngin`,`nomEquipements`,  `description`, `ImageAnom1` ,`ImageAnom2`, `ImageAnom3`, `statut`,`dernierRevision`, `km_reel` , `heure_jour`, `horametre`, `prochainRevision`,`image` 
        ...
        'SELECT `engins`.`id_Engins`, `nomTypes`, `nomClients` , `numeroEngin`,`nomEquipements`, /*  `anomalies`.`description`, `ImageAnom1` ,`ImageAnom2`, `ImageAnom3`,  */`nomStatut`,`dernierRevision`, `km_reel` , `heure_jour`, `horametre`, `prochainRevision`,`image`, `engins`.`id_types` 
        FROM `engins` 
     JOIN `clients`
     JOIN `types` 
     JOIN `equipements`
    ...
    public function getInfoByEngin() {
       ...
       public function getEnginsInfo() {
        $getEnginsInfoQuery = $this->db->prepare(
            'SELECT `id_types`, `numeroEngin`,`id_equipements`, `id_statut`,`dernierRevision`, `km_reel` , `heure_jour`, `horametre`, `prochainRevision`,`id_Clients`, `engins`.`image`
            FROM `engins`
            WHERE `id_Engins` = :id_Engins'
        );
